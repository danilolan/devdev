...
impl TileMap {
    ...
    fn check_if_tile_is_empty(&self, index: [usize; 2]) -> bool {
        ...
        if !self.tiles[index[0]][index[1]].is_open {
            return false;
        }
        if !self.tiles[index[0]][index[1]].entity.is_some() {
            return false;
        }
    ...
    fn world_pos_to_tile_index(&self, position: Vec3) -> Result<[usize; 2], &'static str> {
        if position.x > self.tiles.len() as f32 * self.tile_size
            || position.z > self.tiles[0].len() as f32 * self.tile_size
        {
            return Err("Index is greater than tilemap size.");
        }
...
fn handle_tilemap(mut tilemap: ResMut<TileMap>) {}

fn draw_tilemap(mut gizmos: Gizmos, mut tilemap: ResMut<TileMap>) {
    let map_width = tilemap.tiles.len();
    let map_height = tilemap.tiles[0].len();

    let light = Color::Rgba {
        red: 0.0,
        green: 0.0,
        blue: 0.0,
        alpha: 0.2,
