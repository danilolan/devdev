...
fn show_colliders(colliders_q: Query<(&Transform, &Collider), With<Collider>>, mut gizmos: Gizmos) {
    if !SHOW_COLLIDER {
        return;
    }
    ...
    for (transform, collider) in colliders_q.iter() {
        ...
        let size = Vec3::new(
            cubo.raw.half_extents[0] * 2.0,
            cubo.raw.half_extents[1] * 2.0,
            cubo.raw.half_extents[2] * 2.0,
        );
